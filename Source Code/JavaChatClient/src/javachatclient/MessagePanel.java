...
public abstract class MessagePanel extends JPanel {
    ...
    public void drawMessage(String message, SimpleAttributeSet keyword) {
        ...
        try {
            ...
            if (!GUI.currentView.equals(this.getTitle())) {

                synchronized (Connection.MUTEX) {

                    final String _tabTitle = this.getTitle();
                    final int _tabIndex = GUI.tabbedPane.indexOfTab(this.getTitle());

                    //System.err.println("Title is: " + this.getTitle() + " . Index is: " + _tabIndex);

                    final Color _savedForeground = GUI.tabbedPane.getForeground();
                    final Color _savedBackground = GUI.tabbedPane.getBackground();
                    ...
                    if (!timer.isRunning()) { // If timer has finished running, create a new one.
                        ...
                        timer = new Timer(500, new ActionListener() { // Requirement: import javax.swing.Timer;
                            ...
                            @Override
                            public void actionPerformed(ActionEvent e) {
                                if (System.currentTimeMillis() < end && !GUI.currentView.equals(_tabTitle)) {
                                    if (on) {
                                        if (_foreground != null) {
                                            GUI.tabbedPane.setForegroundAt(_tabIndex, _foreground);
                                        }
                                        if (_background != null) {
                                            GUI.tabbedPane.setBackgroundAt(_tabIndex, _background);
                                        }
                                    } else {
                                        if (_savedForeground != null) {
                                            GUI.tabbedPane.setForegroundAt(_tabIndex, _savedForeground);
                                        }
                                        if (_savedBackground != null) {
                                            GUI.tabbedPane.setBackgroundAt(_tabIndex, _savedBackground);
                                        }
                                    }
                                    repaint();
                                    on = !on;
